...
async function customNotification(pid, notiMsg) {
  ...
  await bbgm.logEvent({
    type: "Progs",
    text: `<a href="${bbgm.helpers.leagueUrl(["player", p.pid])}">${
      p.firstName
    } ${p.lastName}</a> Prog Details:<br/>${notiMsg}.`,
    ...
    if (p.note !== "classic" && p.draft.year !== seasonYr) {
      ...
      if (p.ratings.length > 1) {
        ...
        if (age > 25 && age < 31) {
          ...
          await notification(
            p.pid,
            `PER: ${per.toFixed(2)}<br/>Prog Range: ${progRange} [25-30]`
          );
        }
        ...
        // # 31 to 34
          ...
          await notification(
            p.pid,
            `PER: ${per.toFixed(2)}<br/>Prog Range: ${progRange} [31-35]`
          );
        }
        ...
        // # 35 and beyond
          ...
          await notification(
            p.pid,
            `PER: ${per.toFixed(2)}<br/>Prog Range: ${progRange} [35+]`
          );
        }
        const keys = [
          "diq",
